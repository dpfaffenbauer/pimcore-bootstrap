<div class="row">
    ...
    <div
        id="<?= $randomid ?>" <?php echo $this->editmode ? '' : 'class="carousel slide carousel-fade" data-ride="carousel" data-interval="6500"' ?>>
        <!-- Indicators -->
            ...
            <?php for ($i = 0; $i < $slides; $i++) { ?>
                <div class="item clearfix <?= $i == 0 ? "active" : "" ?>">
                    <?php if ($this->editmode) { ?>
                        <h2>Slide <?= ($i + 1) ?></h2>
                    <?php } ?>
                    <?= $this->areablock("s-$i", array( "params" => array("image" => array("class" => "pull-right")))); ?>
                </div>
            <?php } ?>
        </div>
    </div>
